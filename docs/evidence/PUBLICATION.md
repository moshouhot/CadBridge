...
### 2.5b 历史中仍存在的标识符：精确范围（已核实）

**`main` 与 tag `baseline/pre-sourcery-audit` 仍然含有该标识符。** 早前用
`git grep -I` 得出的"已清理干净"是**假阴性**：`-I` 会跳过被判定为二进制的文件，而泄漏
恰好就在 UTF-16 编码的 `.raw` 文件里。改用按编码逐个字节扫描后，精确范围如下：

| ref / commit | 含标识符的文件 | 编码 |
|---|---|---|
| `main` (`592c6ff`)，即 tag `baseline/pre-sourcery-audit` | `docs/evidence/P1/.../T01-1/matrix/2016-legacy.raw` | UTF-16LE，1 处 |
| 同上 | `docs/evidence/P1/.../T01-1/matrix/2016-modern.raw` | UTF-16LE，1 处 |
| `1925b68`、`cac0c37`、`4c7dc5b`（审计分支） | 上述 2 个 `.raw` **加上** `docs/evidence/P2/20260919T154319Z/build-modern.txt`（24 处） | 混合 |
| `8b13155` 及之后（含 `HEAD`） | **无** | — |

即：**当前 `HEAD` 干净，但 `main` 及其后的 3 个提交不干净**。这 2 个 `.raw` 文件是在第 2 次
泄漏时才发现并修复的（见 §2.6），修复发生在审计分支上，**未**重写 `main`。

**本会话未再重写历史。** 原因：重写 `main` 会改变已发布的 tag `baseline/pre-sourcery-audit`
的指向，而该 tag 正是本审计的**回退目标**；在未得到项目所有者明确决定前，不应破坏它。
是否重写 `main`、是否改 tag，属于需要人工决策的事项，已列入报告的未完成项。
...
对**已提交的 blob**做**按编码的字节扫描**，而不是只扫工作区文本文件，也**不要**用
`git grep -I` —— `-I` 会跳过二进制/UTF-16 文件，而本仓库的证据里恰好有 UTF-16LE 内容，
使用 `-I` 会产生**假阴性**（本审计中实际发生过一次）。

正确做法：对每个 blob 分别用 UTF-8 / UTF-16LE / UTF-16BE / GB18030 编码的字节模式匹配。

本机标识符的专用检查（已集成到 `selftest.sh`，未设环境变量时显示 `SKIP` 而非 `PASS`）：
