...
### S11–S12 — 第四轮 Review 又发现 2 个真实缺陷（均已复现并整改）

第四轮 Review 针对第三轮修复提交，又找到 2 个真实缺陷：

| # | 缺陷 | 复现结果（修复前） |
|---|---|---|
| S11 | 写入失败时若 `os.unlink` **清理也失败**，该异常被吞掉 —— 留下 `manifest.json.*.tmp` 残留，却仍声称“失败不留临时文件” | 残留确认，且无任何报告 |
| S12 | 脱敏用 `errors="replace"` 解码再编码，会**重写文件中所有畸形序列**（不只是标识符）：UTF-16 证据末尾的孤立代理对字节 `00 d8` 变成 `fd ff` | 字节被改，脱敏“静默篡改无关证据字节” |

**整改**：
- 清理失败**上报到 stderr**（原异常仍继续传播），不再静默丢弃。
- 脱敏改为**字节级替换**：先校验文件可解码（用于确定标识符字节模式并拒绝不可扫描文件），
  只替换标识符的编码字节，然后**校验**“写出的字节 == 原文仅替换了标识符片段”；校验失败则
  **拒绝写入**。畸形序列不再被触碰，含畸形序列的文件被**拒绝**而非部分重写。
- **修 S12 时又发现第三个缺陷**：字节模式用了**检测**用的编码，`"utf-8-sig".encode()` 会加上
  BOM，导致 UTF-8/GB18030 文件里的标识符**从未被匹配到**。检测编码与模式编码现已分离
  （这个缺陷被已有的编码夹具抓到 —— 正是它们存在的意义）。

> 新增 3 项 selftest 检查都走真实代码路径：清理失败测试同时注入 replace 与 unlink 失败并断言
> 失败被上报且上一份 manifest 存活；畸形 UTF-16 测试断言文件被拒且字节不变；字节级脱敏测试断言
> 周边文本存活、仅标识符消失。
...
| 项目 | 命令 | 结果 |
|---|---|---|
| 工具自检（历史证据：`8074c60`） | `bash tools/selftest.sh` | **53 passed, 0 failed**，exit 0 |
| 工具自检（当前 HEAD；新增 Codex findings 回归后） | `bash tools/selftest.sh` | **PENDING LOCAL RE-VERIFICATION**；不得继承旧 SHA 计数 |
...
| 负向测试：清理失败被吞（S11） | 同时注入 replace/unlink 失败 | **检出**（上报 + manifest 存活） |
| 负向测试：畸形 UTF-16 被静默重写（S12） | `--check` 对含孤立代理对的文件 | **检出**（拒绝 + 字节不变） |
| 负向测试：字节级脱敏只改标识符 | 校验周边文本 | **检出**（前后文存活） |
